...
def load_entries(path: Path, layout: dict[str, RoleLayout]) -> list[Entry]:
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        for line_no, row in enumerate(csv.reader(f, delimiter="\t"), 1):
            ...
            # New Dubeol physical layout intentionally permits same-finger
            # abbreviation chords. The user may bridge adjacent keys with one
            # finger; do not rewrite or reject canonical logical-role masks.
