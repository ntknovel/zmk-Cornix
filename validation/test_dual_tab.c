...
int main(void){
 ...
 reset_all();cornix_steno_dual_pressed(44,CST_DUAL_VOWEL,CST_R_V_EO,SPACE,0,0);cornix_steno_dual_released(44,50);expect("VEO Space",SPACE);
 reset_all();cornix_steno_dual_pressed(43,CST_DUAL_VOWEL,CST_R_V_U,ENTER,0,0);cornix_steno_dual_released(43,80);expect("VU solo-hold direct",N);
 reset_all();cornix_steno_dual_pressed(44,CST_DUAL_VOWEL,CST_R_V_EO,SPACE,0,0);cornix_steno_dual_released(44,80);expect("VEO solo-hold direct",J);
 ...
 puts("Cornix STENO v2.2.2 80ms vowel tri-state/one-shot-Tab tests: PASS");
 return EXIT_SUCCESS;
}
